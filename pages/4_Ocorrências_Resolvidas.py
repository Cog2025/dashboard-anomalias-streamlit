...
# [NOVO] CSS Inteligente apenas para KPIs
utils.render_page_config_and_css()

# [RESTAURADO] CSS Original dos Botões e Cards Verdes
st.markdown(
    """
<style>
...
/* Botões padrão (VERDES) - Restaurado */
.stButton button{
...
/* Card Verde de Detalhes (Original) */
.card-container{
...
if not df.empty and "Desligamento" in df.columns:
    df["Desligamento"] = pd.to_datetime(df["Desligamento"], errors="coerce")

# KPIs do topo
count_resolvidos_deslig = df[(df["Categoria"] == "DESLIGAMENTOS") & (~df["Normalização"].isna())].shape[0] if not df.empty else 0
count_resolvidos_equip = df[(df["Categoria"] == "EQUIPAMENTOS") & (~df["Normalização"].isna())].shape[0] if not df.empty else 0
...
# Inicialização de filtros (mantida)
...
# Estados UI (mantendo lógica original)
if "ui_filtros_anos" not in st.session_state: st.session_state.ui_filtros_anos = st.session_state.filtros_anos.copy()
if "ui_filtros_meses" not in st.session_state: st.session_state.ui_filtros_meses = st.session_state.filtros_meses.copy()
if "ui_filtros_dias" not in st.session_state: st.session_state.ui_filtros_dias = st.session_state.filtros_dias.copy()
if "ui_categoria_top" not in st.session_state: st.session_state.ui_categoria_top = st.session_state.categoria_top
if "ui_filtros_clientes" not in st.session_state: st.session_state.ui_filtros_clientes = st.session_state.filtros_clientes.copy()
if "ui_filtros_ugs" not in st.session_state: st.session_state.ui_filtros_ugs = st.session_state.filtros_ugs.copy()
if "ui_filtros_tipos" not in st.session_state: st.session_state.ui_filtros_tipos = st.session_state.filtros_tipos.copy()
if "ui_filtros_ativos" not in st.session_state: st.session_state.ui_filtros_ativos = st.session_state.filtros_ativos.copy()
if "ui_filtros_ocorrencias" not in st.session_state: st.session_state.ui_filtros_ocorrencias = st.session_state.filtros_ocorrencias.copy()
...
if not df.empty:
    ...
    st.session_state.ui_categoria_top = st.radio("Categoria:", options=["Ambas", "DESLIGAMENTOS", "EQUIPAMENTOS"], horizontal=True, label_visibility="collapsed", index=["Ambas", "DESLIGAMENTOS", "EQUIPAMENTOS"].index(st.session_state.ui_categoria_top))
    ...
    with col_ano:
        with st.container(border=True):
            ...
            with btn_ano1:
                clicked_sel_ano = st.button("Sel. Todos", key="sel_ano_res", use_container_width=True, on_click=marcar_loading, args=("cb_ano_", anos_disponiveis, "ui_filtros_anos", True))
            with btn_ano2:
                clicked_des_ano = st.button("Desmarcar", key="des_ano_res", use_container_width=True, on_click=marcar_loading, args=("cb_ano_", anos_disponiveis, "ui_filtros_anos", False))
    ...
    with col_mes:
        with st.container(border=True):
            ...
            with btn_mes1:
                clicked_sel_mes = st.button("Sel. Todos", key="sel_mes_res", use_container_width=True, on_click=marcar_loading, args=("cb_mes_", meses_disponiveis, "ui_filtros_meses", True))
            with btn_mes2:
                clicked_des_mes = st.button("Desmarcar", key="des_mes_res", use_container_width=True, on_click=marcar_loading, args=("cb_mes_", meses_disponiveis, "ui_filtros_meses", False))
    ...
    # Dia
    # (Lógica simplificada para dias disponiveis baseada nos anos/meses UI)
                ...
                for i, dia in enumerate(range(1, 32)):
                    with dias_cols[i % 7]:
                        if dia in dias_disponiveis:
                            st.checkbox(str(dia), key=f"cb_dia_{dia}", value=(dia in st.session_state.ui_filtros_dias))
                        else:
                            st.checkbox(str(dia), key=f"cb_dia_{dia}", disabled=True)
            
            btn_dia1, btn_dia2 = st.columns(2)
            with btn_dia1:
                clicked_sel_dia = st.button("Sel. Todos", key="sel_dia_res", use_container_width=True, on_click=marcar_loading, args=("cb_dia_", list(range(1, 32)), "ui_filtros_dias", True, set(dias_disponiveis)))
            with btn_dia2:
                clicked_des_dia = st.button("Desmarcar", key="des_dia_res", use_container_width=True, on_click=marcar_loading, args=("cb_dia_", list(range(1, 32)), "ui_filtros_dias", False, set(dias_disponiveis)))
...
# Filtros Adicionais (Restaurado Sel. Todos / Desmarcar)
...
with col_cliente:
    with st.container(border=True):
        st.write("Cliente")
        cli_opts = sorted([v for v in df_ref["Cliente"].unique() if v and v!="-"])
        st.session_state.ui_filtros_clientes = [x for x in st.session_state.ui_filtros_clientes if x in cli_opts]
        
        btn_cli1, btn_cli2 = st.columns(2)
        with btn_cli1:
            if st.button("Sel. Todos", key="cli_sel_all_res", use_container_width=True): st.session_state.ui_filtros_clientes = cli_opts[:]
        with btn_cli2:
            if st.button("Desmarcar", key="cli_clear_res", use_container_width=True): st.session_state.ui_filtros_clientes = []
            
        st.session_state.ui_filtros_clientes = st.multiselect("", options=cli_opts, default=st.session_state.ui_filtros_clientes, label_visibility="hidden")

# UG
with col_ug:
    with st.container(border=True):
        st.write("UG")
        ug_opts = sorted([v for v in df_ref["UG"].unique() if v and v!="-"])
        st.session_state.ui_filtros_ugs = [x for x in st.session_state.ui_filtros_ugs if x in ug_opts]
        
        btn_ug1, btn_ug2 = st.columns(2)
        with btn_ug1:
            if st.button("Sel. Todos", key="ug_sel_all_res", use_container_width=True): st.session_state.ui_filtros_ugs = ug_opts[:]
        with btn_ug2:
            if st.button("Desmarcar", key="ug_clear_res", use_container_width=True): st.session_state.ui_filtros_ugs = []
            
        st.session_state.ui_filtros_ugs = st.multiselect("", options=ug_opts, default=st.session_state.ui_filtros_ugs, label_visibility="hidden")

# Tipo
with col_tipo:
    with st.container(border=True):
        st.write("Tipo de Ocorrência")
        tip_opts = sorted([v for v in options_from(df_ref["Tipo de ocorrência"]) if v and v!="-"])
        st.session_state.ui_filtros_tipos = [x for x in st.session_state.ui_filtros_tipos if x in tip_opts]
        
        btn_tipo1, btn_tipo2 = st.columns(2)
        with btn_tipo1:
            if st.button("Sel. Todos", key="tipo_sel_all_res", use_container_width=True): st.session_state.ui_filtros_tipos = tip_opts[:]
        with btn_tipo2:
            if st.button("Desmarcar", key="tipo_clear_res", use_container_width=True): st.session_state.ui_filtros_tipos = []
        
        st.session_state.ui_filtros_tipos = st.multiselect("", options=tip_opts, default=st.session_state.ui_filtros_tipos, label_visibility="hidden")

# Ativo
with col_ativo:
    with st.container(border=True):
        st.write("Ativo")
        atv_opts = sorted([v for v in options_from(df_ref["Ativo"]) if v and v!="-"])
        st.session_state.ui_filtros_ativos = [x for x in st.session_state.ui_filtros_ativos if x in atv_opts]
        
        btn_atv1, btn_atv2 = st.columns(2)
        with btn_atv1:
            if st.button("Sel. Todos", key="ativo_sel_all_res", use_container_width=True): st.session_state.ui_filtros_ativos = atv_opts[:]
        with btn_atv2:
            if st.button("Desmarcar", key="ativo_clear_res", use_container_width=True): st.session_state.ui_filtros_ativos = []
        
        st.session_state.ui_filtros_ativos = st.multiselect("", options=atv_opts, default=st.session_state.ui_filtros_ativos, label_visibility="hidden")

# Ocorrência
with col_ocorrencia:
    with st.container(border=True):
        st.write("Ocorrência")
        ocr_opts = sorted([v for v in options_from(df_ref["Ocorrência"]) if v and v!="-"])
        st.session_state.ui_filtros_ocorrencias = [x for x in st.session_state.ui_filtros_ocorrencias if x in ocr_opts]
        
        btn_ocr1, btn_ocr2 = st.columns(2)
        with btn_ocr1:
            if st.button("Sel. Todos", key="ocr_sel_all_res", use_container_width=True): st.session_state.ui_filtros_ocorrencias = ocr_opts[:]
        with btn_ocr2:
            if st.button("Desmarcar", key="ocr_clear_res", use_container_width=True): st.session_state.ui_filtros_ocorrencias = []
        
        st.session_state.ui_filtros_ocorrencias = st.multiselect("", options=ocr_opts, default=st.session_state.ui_filtros_ocorrencias, label_visibility="hidden")
...
# Aplicação dos filtros (usando variaveis reais, nao UI)
anos_sel = st.session_state.filtros_anos
meses_sel = st.session_state.filtros_meses
dias_sel = st.session_state.filtros_dias

all_anos = set(anos_sel) == set_anos_disp and len(set_anos_disp) > 0
all_meses = set(meses_sel) == set_meses_disp and len(set_meses_disp) > 0
all_dias = set(dias_sel) == set_dias_disp and len(set_dias_disp) > 0

m_ano = df["Ano"].isin(anos_sel) if anos_sel and not all_anos else pd.Series(True, index=df.index)
m_mes = df["Mês"].isin(meses_sel) if meses_sel and not all_meses else pd.Series(True, index=df.index)
m_dia = df["Dia"].isin(dias_sel) if dias_sel and not all_dias else pd.Series(True, index=df.index)

m_cat = df["Categoria"].isin(st.session_state.filtros_categorias)
if st.session_state.get("categoria_top_aplicada") in ("DESLIGAMENTOS", "EQUIPAMENTOS"):
    m_cat = m_cat & (df["Categoria"] == st.session_state.categoria_top_aplicada)

m_cli = matches_any_canon(df["Cliente"], st.session_state.filtros_clientes) if st.session_state.filtros_clientes else pd.Series(True, index=df.index)
m_ug = df["UG"].isin(st.session_state.filtros_ugs) if st.session_state.filtros_ugs else pd.Series(True, index=df.index)
m_tip = matches_any_canon(df["Tipo de ocorrência"], st.session_state.filtros_tipos) if st.session_state.filtros_tipos else pd.Series(True, index=df.index)
m_atv = matches_any_canon(df["Ativo"], st.session_state.filtros_ativos) if st.session_state.filtros_ativos else pd.Series(True, index=df.index)
m_ocr = matches_any_canon(df["Ocorrência"], st.session_state.filtros_ocorrencias) if st.session_state.filtros_ocorrencias else pd.Series(True, index=df.index)
...
if not df_resolvidas.empty:
    ...
    for i in range(0, len(rows), num_cols):
        try: cols = st.columns(num_cols, gap="small")
        except: cols = st.columns(num_cols)
