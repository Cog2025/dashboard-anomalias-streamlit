...
def carregar_dados(cb):
    try:
        ...
        for col in df.columns:
            ...
            if col_upper in mapa_renomear:
                renomear_final[col] = mapa_renomear[col_upper]
...
# Verifica se a coluna crítica existe
if 'Normalização' not in df_todos.columns:
    st.error("Erro: A coluna 'Normalização' não foi encontrada. Verifique o cabeçalho da planilha.")
    st.stop()
...
if not df_filt.empty:
    ...
    st.info("Nenhuma histórico para os filtros selecionados.")