...
def options_from(series: pd.Series) -> list:
    # Lógica original que inclui o traço "-"
    series = series.astype(str).map(_collapse_spaces)
    unique_vals = sorted([x for x in series.unique() if x and x.lower() != "nan" and x != ""])
    return ["-"] + unique_vals
...
<style>
    ...
    .card-container {
        background-color: #089641; color: white; padding: 15px;
...
with st.sidebar:
    ...
    with st.expander("📅 Anos", expanded=True):
        if anos_disp:
            ...
            b2.button('Nada', key='des_ano', on_click=marcar_e_loading, args=('cb_ano_', anos_disp, 'filtros_anos', False))
    ...
    with st.expander("📆 Meses", expanded=False):
        ...
        b2.button('Nada', key='des_mes', on_click=marcar_e_loading, args=('cb_mes_', meses_cronologicos, 'filtros_meses', False))
    ...
    with st.expander("numeric Dias", expanded=False):
        ...
        # Grid para dias (mais compacto na sidebar)
        cols_d = st.columns(4) 
        ...
        b2.button('Nada', key='des_dia', on_click=marcar_e_loading, args=('cb_dia_', list(range(1,32)), 'filtros_dias', False))
    ...
    # Filtros Adicionais (Multiselects na Sidebar)
    # Inicializa
    for k in ['filtros_clientes', 'filtros_ugs', 'filtros_tipos', 'filtros_ativos', 'filtros_ocorrencias']:
        if k not in st.session_state: st.session_state[k] = []

    # Função Helper Render Sidebar
    def render_sidebar_filter(label, key, options, sel_key, des_key):
        with st.expander(label):
            ...
            if b1.button('Todos', key=sel_key):
                 ...
                 st.rerun()
            if b2.button('Nada', key=des_key):
                 st.session_state[key] = []
                 st.rerun()
            
            st.session_state[key] = st.multiselect("Selecione", opts_clean, default=[x for x in st.session_state[key] if x in opts_clean], key=f"ms_{key}")
    ...
    if not df_todos_dados.empty:
        ...
        render_sidebar_filter("Clientes", 'filtros_clientes', cli_opts, 'sel_cli', 'des_cli')
        render_sidebar_filter("UGs", 'filtros_ugs', ug_opts, 'sel_ug', 'des_ug')
        render_sidebar_filter("Tipo", 'filtros_tipos', options_from(df_todos_dados['Tipo de ocorrência']), 'sel_tip', 'des_tip')
        render_sidebar_filter("Ativo", 'filtros_ativos', options_from(df_todos_dados['Ativo']), 'sel_atv', 'des_atv')
        render_sidebar_filter("Ocorrência", 'filtros_ocorrencias', options_from(df_todos_dados['Ocorrência']), 'sel_ocr', 'des_ocr')
...
df_filt = df_todos_dados[m_ano & m_mes & m_dia & m_cat & m_cli & m_ug & m_tip & m_atv & m_ocr].copy()
